...
void SkinSelect::drawSkinCell(int col, int row, bool unlocked, bool isEquipped, bool isSelected) {
    ...
    if (unlocked) {
        ...
        if (isEquipped) {
            const char* eq = "EQUIPPED";
            int eqW = MeasureText(eq, 12);
            DrawText(eq, cell.x + cell.width / 2 - eqW / 2, cell.y + 5, 12, GOLD);
        }
    } else {
...
void SkinSelect::Draw() {
    ...
    DrawText(TextFormat("Skins: %d/15", unlocked), 1080 - MeasureText(TextFormat("Skins: %d/15", unlocked), 18) - 20, 100, 18, ColorAlpha(WHITE, 0.6f));
