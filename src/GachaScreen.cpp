...
void GachaScreen::LoadContent() {
    ...
    if (!sndLoaded) {
        if (FileExists("assets/gacha/SPIN_WHEELS.wav")) sndSpinWheels = LoadSound("assets/gacha/SPIN_WHEELS.wav");
        if (FileExists("assets/gacha/ZONK.mp3")) sndZonk = LoadSound("assets/gacha/ZONK.mp3");
        if (FileExists("assets/gacha/COIN.wav")) sndCoin = LoadSound("assets/gacha/COIN.wav");
        if (FileExists("assets/gacha/FREE_SPIN.wav")) sndFreeSpin = LoadSound("assets/gacha/FREE_SPIN.wav");
        if (FileExists("assets/gacha/COMMON_UNCOMMON.wav")) sndCommon = LoadSound("assets/gacha/COMMON_UNCOMMON.wav");
        if (FileExists("assets/gacha/RARE.wav")) sndRare = LoadSound("assets/gacha/RARE.wav");
        if (FileExists("assets/gacha/EPIC.wav")) sndEpic = LoadSound("assets/gacha/EPIC.wav");
        if (FileExists("assets/gacha/LEGENDARY.wav")) sndLegendary = LoadSound("assets/gacha/LEGENDARY.wav");
        sndLoaded = true;
    }
...
void GachaScreen::UnloadContent() {
    ...
    if (sndLoaded) {
        auto unload = [](Sound& s) { if (s.stream.buffer != nullptr) UnloadSound(s); };
        unload(sndSpinWheels); unload(sndZonk); unload(sndCoin); unload(sndFreeSpin);
        unload(sndCommon); unload(sndRare); unload(sndEpic); unload(sndLegendary);
        sndLoaded = false;
    }
    contentLoaded = false;
}
...
void GachaScreen::StartSpin() {
    ...
    if (sndSpinWheels.stream.buffer != nullptr) {
        if (spinSoundPlaying) StopSound(sndSpinWheels);
        PlaySound(sndSpinWheels);
...
void GachaScreen::PlayRewardSound() {
    switch (rewardType) {
        case GachaItemType::ZONK:
            if (sndZonk.stream.buffer != nullptr) PlaySound(sndZonk); break;
        case GachaItemType::COIN:
            if (sndCoin.stream.buffer != nullptr) PlaySound(sndCoin); break;
        case GachaItemType::FREE_SPIN:
            if (sndFreeSpin.stream.buffer != nullptr) PlaySound(sndFreeSpin); break;
        case GachaItemType::SKIN:
            switch (rewardRarity) {
                case 1: case 2:
                    if (sndCommon.stream.buffer != nullptr) PlaySound(sndCommon); break;
                case 3:
                    if (sndRare.stream.buffer != nullptr) PlaySound(sndRare); break;
                case 4:
                    if (sndEpic.stream.buffer != nullptr) PlaySound(sndEpic); break;
                case 5:
                    if (sndLegendary.stream.buffer != nullptr) PlaySound(sndLegendary); break;
            } break;
    }
}
