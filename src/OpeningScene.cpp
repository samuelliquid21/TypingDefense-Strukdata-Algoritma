...
void OpeningScene::Init() {
    ...
    // Textures
    am.LoadTex("nova",          "assets/img/nova.jpg");
    am.LoadTex("captain_voss",  "assets/img/captain_voss.jpg");
    am.LoadTex("spaceship_ace", "assets/img/spaceship_ace.png");

    // Dialog audio
    am.LoadSnd("voss_01",   "assets/sound/voss_01.mp3");
    am.LoadSnd("voss_02",   "assets/sound/voss_02.mp3");
    am.LoadSnd("voss_03",   "assets/sound/voss_03.mp3");
    am.LoadSnd("voss_04",   "assets/sound/voss_04.mp3");
    am.LoadSnd("voss_05",   "assets/sound/voss_05.mp3");
    am.LoadSnd("voss_06",   "assets/sound/voss_06.mp3");
    am.LoadSnd("voss_07",   "assets/sound/voss_07.mp3");
    am.LoadSnd("voss_08",   "assets/sound/voss_08.mp3");
    am.LoadSnd("voss_09",   "assets/sound/voss_09.mp3");
    am.LoadSnd("voss_10",   "assets/sound/voss_10.mp3");
    am.LoadSnd("voss_11",   "assets/sound/voss_11.mp3");
    am.LoadSnd("ace_01",    "assets/sound/ace_01.mp3");
    am.LoadSnd("ace_02",    "assets/sound/ace_02.mp3");
    am.LoadSnd("ace_03",    "assets/sound/ace_03.mp3");
    am.LoadSnd("ace_04",    "assets/sound/ace_04.mp3");
    am.LoadSnd("ace_05",    "assets/sound/ace_05.mp3");
    am.LoadSnd("ace_06",    "assets/sound/ace_06.mp3");
    am.LoadSnd("nova_01",   "assets/sound/nova_01.mp3");
    am.LoadSnd("nova_02",   "assets/sound/nova_02.mp3");
    am.LoadSnd("system_1",  "assets/sound/system_1.mp3");
    am.LoadSnd("system_02", "assets/sound/system_02.mp3");
    ...
    if (am.HasSnd("opening_music")) am.StopSnd("opening_music");
}
...
void OpeningScene::PlayDialogAudio() {
    ...
    if (AssetManager::Get().HasSnd(key)) {
        AssetManager::Get().PlaySnd(key);
        audioPlayed = true;
    }
}
...
void OpeningScene::FadeOutOpeningMusic(float dt) {
    ...
    if (openingMusicVol <= 0.0f) {
        ...
        if (am.HasSnd("opening_music")) {
            SetSoundVolume(am.GetSnd("opening_music"), openingMusicVol);
        }
    }
}
...
void OpeningScene::NextDialog() {
    ...
    if (currentIndex < (int)dialogs.size()) {
        const std::string& key = dialogs[currentIndex].audioKey;
        if (!key.empty() && AssetManager::Get().HasSnd(key))
            AssetManager::Get().StopSnd(key);
    }
...
void OpeningScene::SkipAll() {
    // Stop semua audio
    for (auto& d : dialogs) {
        if (!d.audioKey.empty() && AssetManager::Get().HasSnd(d.audioKey))
            AssetManager::Get().StopSnd(d.audioKey);
    }
    openingMusicFading = true;
    isSkipped          = true;
    isFadingOut        = true;
...
void OpeningScene::Update(bool& doneOpening) {
    ...
    if (isDialogTransiting) {
        ...
        if (dialogTransTimer < 0.2f) {
            ...
            charAlpha = 1.0f - t;
        } else if (dialogTransTimer < 0.22f) {
            NextDialog();
            dialogBoxAlpha = 0.0f;
            charAlpha = 0.0f;
...
void OpeningScene::DrawPortrait() {
    ...
    if (key.empty() || !am.HasTex(key)) return;
    Texture2D& tex = am.GetTex(key);
