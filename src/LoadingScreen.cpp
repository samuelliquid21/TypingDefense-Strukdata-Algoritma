...
void LoadingScreen::DrawBackground() {
    ...
    if (storySlides.empty()) {
        ...
        if (!am.hasTexture("loading_bg")) {
            am.loadTexture("loading_bg", "assets/img/loading_bg.png");
        }
        Texture2D bg = am.getTexture("loading_bg");
...
void LoadingScreen::Update(bool& doneLoading) {
    ...
    if (!am.hasSound("opening_music")) {
        am.loadSound("opening_music", "assets/sound/opening.mp3");
        PlaySound(am.getSound("opening_music"));
    }
